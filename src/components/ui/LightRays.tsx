...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  return m ? [parseInt(m[1], 16) / 255, parseInt(m[2], 16) / 255, parseInt(m[3], 16) / 255] as [number, number, number] : [1, 1, 1];
};
...
interface LightRaysProps {
  raysOrigin?: RaysOrigin; raysColor?: string; raysSpeed?: number;
  lightSpread?: number; rayLength?: number; pulsating?: boolean;
  fadeDistance?: number; saturation?: number; followMouse?: boolean;
  mouseInfluence?: number; noiseAmount?: number; distortion?: number;
  className?: string;
}
...
void main(){ vec4 color; mainImage(color,gl_FragCoord.xy); gl_FragColor=color; }`;
      ...
      cleanupRef.current = () => {
        ...
        try { const c = renderer.gl.canvas; const ext = renderer.gl.getExtension('WEBGL_lose_context'); if (ext) ext.loseContext(); if (c?.parentNode) c.parentNode.removeChild(c); } catch {}
  ...
  useEffect(() => {
    const u = uniformsRef.current; if (!u || !containerRef.current || !rendererRef.current) return;
    ...
    const { anchor, dir } = getAnchorAndDir(raysOrigin, w * rendererRef.current.dpr, h * rendererRef.current.dpr);
  ...
  useEffect(() => {
    ...
    if (followMouse) { window.addEventListener('mousemove', h); return () => window.removeEventListener('mousemove', h); }
  }, [followMouse]);
