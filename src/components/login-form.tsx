...
export function LoginForm({ className, visible, ...props }: React.ComponentProps<'div'> & { visible?: boolean }) {
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      supabase.auth.signInWithOAuth({ provider: 'github', options: { redirectTo: window.location.origin + '/mfa' } })
    } catch (err) {
      setError(err instanceof Error ? err.message : '验证失败')
          ...
          <CardContent className="px-4 sm:px-6">
            {graceLoggingIn ? (
              <div className="py-10 text-center animate-in fade-in-0 duration-300">
                ...
                <p className="mt-4 text-sm font-medium text-muted-foreground">{t('auth.mfaGraceLoggingIn')}</p>
              </div>
            ) : (
                ...
                <div className="grid gap-6">
                  <div className={cn(rowBase, v)} style={{ transitionDelay: '400ms' }}>
                    <div className="flex justify-center gap-4">
                      ...
                      <Button variant="outline" size="icon" type="button" className="rounded-full size-12 border-gray-300 bg-white hover:bg-gray-50 dark:border-white/20 dark:bg-white/5 dark:hover:bg-white/10 backdrop-blur" onClick={handleQrLogin} title="扫码登录">
                        <QrCode className="h-6 w-6 text-gray-800 dark:text-white" />
                      </Button>
                    </div>
                  </div>
