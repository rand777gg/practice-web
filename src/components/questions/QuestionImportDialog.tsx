...
function parseQuestions(text: string, format: 'json' | 'csv'): { questions: ImportedQuestion[]; errors: string[] } {
  ...
  if (format === 'json') {
    ...
    const questions = arr.map((item: Record<string, unknown>) => ({
      ...
      categories: item.categories ? (Array.isArray(item.categories) ? item.categories.map(String) : [String(item.categories)]) : item.category ? [String(item.category)] : undefined,
  ...
  const questions = rows.map((row, i) => {
    ...
    return {
      ...
      categories: obj.categories ? (typeof obj.categories === 'string' ? obj.categories.split(',').map((s: string) => s.trim()).filter(Boolean) : [String(obj.categories)]) : obj.category ? [obj.category] : undefined,
