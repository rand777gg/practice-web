// Browser-safe subset of archify's shared CLI helpers.
//
// Ported from upstream archify/renderers/shared/cli.mjs (commit 1072200, MIT).
// The original module is a CLI shell: it imports node:fs / node:path,
// repository-evidence, engineering-profiles, output-path and brand capture, and
// calls installRendererDiagnosticBoundary() at module scope. Only the pure SVG
// attribute helpers consumed by the workflow compiler are kept here, with
// function bodies preserved verbatim. See vendor/ORIGIN.md for the full patch
// list. `svgRootAttrs` drops the `process.env.ARCHIFY_QUALITY_PROFILE` override
// (browsers have no env); quality now always comes from the authored meta.
