// Browser-safe diagnostics helpers.
//
// Ported from upstream archify/renderers/shared/diagnostics.mjs (commit
// 1072200, MIT) with the node-only pieces removed: the node:fs / node:path
// imports, the `process.env`-driven diagnostic mode (always off here), and the
// CLI uncaughtException boundary (installRendererDiagnosticBoundary /
// fallbackDiagnostic / rendererFailure). Everything the workflow compiler and
// geometry/legend modules call — recordDiagnostic,
// withDiagnosticRecordingSuppressed, throwDiagnosticError,
// throwDiagnosticProblems — is kept with its original body.
